...
결과는 로컬 단일 서버·단일 PostgreSQL 환경에서 얻은 상대 비교다. 배포 환경의 네트워크 지연, DB 사양, 여러 애플리케이션 인스턴스는 포함하지 않았다. 이후에는 동일 조건의 배포 환경에서 재측정하고, 낙관적 락의 재시도 정책과 DB 제약 위반 응답을 함께 검증할 수 있다.
